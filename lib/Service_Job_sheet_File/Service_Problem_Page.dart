...
class Service_Problem_Page extends StatefulWidget {
  const Service_Problem_Page({Key? key}) : super(key: key);

  @override
  State<Service_Problem_Page> createState() => _Service_Problem_PageState();
}

class _Service_Problem_PageState extends State<Service_Problem_Page> {

  TextEditingController service_problem=TextEditingController();
      ...
      children: [
        Row(
          children: [
            ...
            Padding(
              padding:  EdgeInsets.only(left: width/1.455, top: height/21.9),
                ...
                children: [
                  ...
                  StreamBuilder<QuerySnapshot>(
                    stream: FirebaseFirestore.instance
                        .collection("Service_problem").orderBy("timestamp",descending: true)
                        .snapshots(),
                    builder: (context, snapshot) {
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          if(isserach==true&&snapshot.data!.docs[index]["Description"].toString().toLowerCase().startsWith(Username.toLowerCase())){
                            return Row(
                              children: [
  ...
  //controller clear
  Clearall(){
    service_problem.clear();
  }

  Servicecreatefunction(){
    FirebaseFirestore.instance.collection("Service_problem").doc().set({
      "Description":service_problem.text,
...
//delete function
  delete(catid){
    FirebaseFirestore.instance.collection("Service_problem").doc(catid).delete();
  ...
  edidtheitemfunction(docid){
    FirebaseFirestore.instance.collection("Service_problem").doc(docid).update({
      "Description":edititem.text,
    });
  }

  ediditemsetfunction(docid)async{
    var getdatae=await FirebaseFirestore.instance.collection("Service_problem").doc(docid).get();
    Map<String,dynamic>?value=getdatae.data();
    setState(() {
      edititem.text=value!['Description'];
    });
  }
