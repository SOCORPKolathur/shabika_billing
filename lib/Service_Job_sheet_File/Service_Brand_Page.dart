...
class Service_Brand_Page extends StatefulWidget {
  const Service_Brand_Page({Key? key}) : super(key: key);

  @override
  State<Service_Brand_Page> createState() => _Service_Brand_PageState();
}

class _Service_Brand_PageState extends State<Service_Brand_Page> {

  TextEditingController service_Brand=TextEditingController();
                  ...
                  children: [
                    ...
                    StreamBuilder<QuerySnapshot>(
                      stream: FirebaseFirestore.instance
                          .collection("Service_Brand").orderBy("timestamp",descending: true)
                          .snapshots(),
                      builder: (context, snapshot) {
                        ...
                        return ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            if(isserach==true&&snapshot.data!.docs[index]["service brand"].toString().toLowerCase().startsWith(Username.toLowerCase())){
                              return Row(
                                children: [
  ...
  Servicecreatefunction(){
    FirebaseFirestore.instance.collection("Service_Brand").doc().set({
      "service brand":service_Brand.text,
...
//delete function
  delete(catid){
    FirebaseFirestore.instance.collection("Service_Brand").doc(catid).delete();
  ...
  edidtheitemfunction(docid){
    FirebaseFirestore.instance.collection("Service_Brand").doc(docid).update({
      "service brand":edititem.text,
    });
  }

  ediditemsetfunction(docid)async{
    var getdatae=await FirebaseFirestore.instance.collection("Service_Brand").doc(docid).get();
    Map<String,dynamic>?value=getdatae.data();
    setState(() {
      edititem.text=value!['service brand'];
    });
  }
