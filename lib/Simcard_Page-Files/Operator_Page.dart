...
class Opeartor_Page extends StatefulWidget {
  const Opeartor_Page({Key? key}) : super(key: key);

  @override
  State<Opeartor_Page> createState() => _Opeartor_PageState();
}

class _Opeartor_PageState extends State<Opeartor_Page> {
                ...
                children: [
                  ...
                  StreamBuilder<QuerySnapshot>(
                    stream: FirebaseFirestore.instance
                        .collection("Operator").orderBy("timestamp",descending: true)
                        .snapshots(),
                    builder: (context, snapshot) {
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          if(isserach==true&&snapshot.data!.docs[index]["Operatorname"].toString().toLowerCase().
                          ...
                          else if(isserach==false&&snapshot.data!.docs[index]["Operatorname"].toString().toLowerCase().startsWith(Username.toLowerCase())){
                            return Row(
                              children: [
  ...
  category() {
    FirebaseFirestore.instance.collection("Operator").doc().set({
              ...
              SizedBox(
                ...
                child: Column(
                  children: [
                    ...
                    Text("Add a Operator Item Successfully",style: GoogleFonts.openSans(
                        fontWeight: FontWeight.w600,
                        fontSize: width/68.3,
                        color: Colors.white),),
                    SizedBox(height: height/32.85,),
...
//delete function
  delete(catid){
    FirebaseFirestore.instance.collection("Operator").doc(catid).delete();
  ...
  edidtheitemfunction(docid){
    FirebaseFirestore.instance.collection("Operator").doc(docid).update({
      "Operatorname":edititem.text,
    });
  }

  ediditemsetfunction(docid)async{
    var getdatae=await FirebaseFirestore.instance.collection("Operator").doc(docid).get();
    Map<String,dynamic>?value=getdatae.data();
    setState(() {
      edititem.text=value!['Operatorname'];
    });
  }
